...
- get_lead_counts — **聚合数: 总数 + 每人 owned + unassigned pool**, 不返回 lead 详情. args: { since_days?: 1-365 (默认 7), geo?: "cn"|"edu"|"overseas" (按 author_email 域名分类), lead_tier?: "strong"|"normal" }. 返回: { window_days, filters, total, unassigned, per_rep: [{rep_id, name, owned_count}, ...排序: 多到少] }. **什么时候用 (这是关键)**: rep 或 admin 问 "**多少 / 有几条 / 总数 / 谁有多少 / 这周来了多少 cn 的**" 这种**聚合数量**问题. **不要**用 list_leads + 数组长度 (那个 cap=20, 数不对). list_leads 是要"看具体哪几条"用的, get_lead_counts 是要"多少条"用的. 例子: admin 问"cn 这周收到多少个 lead" → get_lead_counts({ since_days: 7, geo: "cn" }) → 直接念 total.
