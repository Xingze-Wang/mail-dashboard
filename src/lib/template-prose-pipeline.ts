...
除了 verdict, 还要给 6 个维度的 1-10 分 (1=很差, 10=完美). 这些分数会跟人类评分横向对比, 是校准信号:

- **politeness** (1-10): 礼貌的拿捏 — 卑微 (低分) ↔ 平等 (高分) ↔ 傲慢 (低分)
- **clarity** (1-10): 30 秒内能不能 get 到"我能不能用上"
- **peer_register** (1-10): 平等同行语气 (10) vs 销售腔 / 推销话术 (1)
- **brand_fit** (1-10): 务实 / 坦然 / 简朴 / 谦逊 四性贴合度
- **factual_accuracy** (1-10): 数字 / program facts 是不是符合 (10=完全符合, 1=与事实严重不符)
- **naturalness** (1-10): 读起来像真人写的 (10) vs LLM 一眼能看出 (1)

打分要诚实, 别都给 8 分. 如果触发任何红线, 对应的维度分应该 ≤ 4.
...
verdict 规则:
- 任一红线触发 → reject (issues 里给具体证据, 对应维度分 ≤ 4)
- 0 红线 + ≤2 个软标准 → pass
- 0 红线 + ≥3 个软标准 → revise
...
export interface EditorReview {
  verdict: "pass" | "revise" | "reject";
  issues: EditorIssue[];
  scores?: EditorScores; // optional — older parses might not have it
  tone_assessment: string;
}

/**
 * Coerce a raw scores object from the LLM into a valid EditorScores.
 * Clamps to 1-10, rounds to int, defaults to 5 (middle) when missing
 * — better to default to "neutral" than to assume worst-case.
 */
function coerceScores(raw: unknown): EditorScores {
  ...
  return {
    ...
    clarity: dim("clarity"),
    peer_register: dim("peer_register"),
    brand_fit: dim("brand_fit"),
...
export async function editParagraph(args: {
  ...
  const tryParse = (raw: string): EditorReview | null => {
    try {
      const clean = raw.trim().replace(/^```(?:json)?\s*/i, "").replace(/\s*```\s*$/i, "");
      const parsed = JSON.parse(clean) as EditorReview & { scores?: unknown };
      if (!parsed.verdict || !Array.isArray(parsed.issues)) return null;
      // Coerce scores. Missing → middle (5s). Out-of-range → clamped.
      // Older versions of the prompt didn't ask for scores, so we
      // tolerate their absence rather than fail-safe to revise.
      return { ...parsed, scores: coerceScores(parsed.scores) };
    } catch {
      return null;
    }
  };
...
export async function craftAndGateProposal(args: {
  ...
  // Persist the editor's multi-dim scores against this template.
  // Best-effort — if the table doesn't exist or insert fails, we
  // don't fail the proposal. The scores are calibration data; the
  // proposal itself is the primary artifact.
  if (review.scores) {
    try {
      ...
      console.error("[template-prose-pipeline] template_ratings insert failed:", e);
    }
  }
