// Agent 16 — pdf-cover for slice-16 only.
// Mirrors strategyPdfCover in scripts/enrich-net.mjs but iterates the
// pre-sliced paper list rather than scanning the full papers table.

import { createClient } from "@supabase/supabase-js";
import { readFileSync, appendFileSync } from "node:fs";

const SLICE_PATH = "/Users/xingzewang/Desktop/mail/scripts/agent-runs/enrich-bg/slice-16.json";
...
async function worker() {
  while (queue.length) {
    ...
    if (scanned % 10 === 0) {
      process.stdout.write(`  ${scanned}/${items.length} (emails=${withEmails}, repos+=${repoUpdates}, new_persons=${newPersons})\r`);
    }
  }
}
...
const summary = {
  ...
  strategy: "pdf-cover",
  scanned,
  wins: repoUpdates + withEmails,
  errors,
  new_persons: newPersons,
  duration_ms,
  total_emails: totalEmails,
  papers_with_emails: withEmails,
