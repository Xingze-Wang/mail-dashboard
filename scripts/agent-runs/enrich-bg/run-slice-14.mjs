// Agent 14 — pdf-cover for slice-14 only.
// Mirrors strategyPdfCover in scripts/enrich-net.mjs but bounded to the
// 200 papers in slice-14.json so we don't step on the other pdf-cover agents.
//
// Behavior (idempotent):
//   - GET first 150KB of arxiv PDF
//   - Extract emails (filter junk domains) -> insert as new persons rows
//     IF email is not already on any person.
//   - Extract hf/github repos -> update papers row only if currently null.
//   - New persons rows for co-author emails (per AGENTS spec: "co-authors
//     are independent identities").

import { createClient } from "@supabase/supabase-js";
import { readFileSync, appendFileSync } from "node:fs";

const SLICE_PATH = "/Users/xingzewang/Desktop/mail/scripts/agent-runs/enrich-bg/slice-14.json";
...
const papers = slice.items;
const start = Date.now();
console.log(`agent ${slice.agent} (pdf-cover): ${papers.length} papers`);
...
async function worker() {
  while (queue.length) {
      ...
      if (emails.length > 0) {
        ...
        for (const email of emails) {
          const { data: dup } = await sb
            .from("persons").select("id").contains("emails", [email]).maybeSingle();
          if (dup) continue;
    ...
    if (scanned % 10 === 0) {
      process.stdout.write(`  ${scanned}/${papers.length} (emails=${withEmails}, repos+=${repoUpdates}, newp=${newPersons})\r`);
    }
  }
}
