...
const sql = readFileSync("migrations/101-agent-scheduled-actions.sql", "utf8");
...
const { data, error: probeErr } = await sb
  ...
  .select("id, kind, cron_expr, status, admin_approved")
  .limit(1);
...
console.log("✓ migration 101 applied cleanly.");
